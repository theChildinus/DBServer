...
FOR %%C in ("%CARBON_HOME%\webapps\ROOT\WEB-INF\plugins\common\synapse-*.jar") DO set CARBON_CLASSPATH=!CARBON_CLASSPATH!;".\webapps\ROOT\WEB-INF\plugins\common\%%~nC%%~xC"
FOR %%C in ("%CARBON_HOME%\webapps\ROOT\WEB-INF\plugins\common\commons-*.jar") DO set CARBON_CLASSPATH=!CARBON_CLASSPATH!;".\webapps\ROOT\WEB-INF\plugins\common\%%~nC%%~xC"
FOR %%C in ("%CARBON_HOME%\lib\commons-*.jar") DO set CARBON_CLASSPATH=!CARBON_CLASSPATH!;".\lib\%%~nC%%~xC"
...
%_RUNJAVA% %JAVA_OPTS% -cp "%CARBON_CLASSPATH%" -Djava.endorsed.dirs=%JAVA_ENDORSED%  org.apache.synapse.commons.security.tool.CipherTool %*
endlocal
:end
