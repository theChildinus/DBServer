...
fd_refresh.bottom = new FormAttachment(table, -3);
...
//if(SysConfig.Instance().getColIsoptional(table.getColumn(columnIndex).getText()) == 1)
//{
//	optionalItems = SysConfig.Instance().getColOptionalItems(table.getColumn(columnIndex).getText());
...
//	if(compare(tmpDocumentDataList,documentDataList))
//	{
//		clear = false;
//	}
//	else
//	{
//		tmpDocumentDataList = documentDataList;
//		clear = true;//当有内容变化的时候才刷新界面
//	}
...
//	else
//	{
//		if(MDataManager.getInstance().getFresh())
//		{
...
//		{
			...
			if(columnNameList.size() != 0 
				...
				//&& !MDataManager.getInstance().getFresh())
			{
				//过滤列项
				shell.getDisplay().syncExec( new Runnable(){ public void run()	{//*****
	...
	protected void okPressed()
	{
		...
		for (int i = 0; i < buttons.length; i++) 
		{
		    if (buttons[i].getSelection())
		     {
		    	columnNameList.add(buttons[i].getText());
		     }
		}	
		setReturnCode(OK);
		close();
...
//--------------------------------------------------------------------------------
	protected Control createDialogArea( Composite parent )
	{
		...
		d7.bottom = new FormAttachment(0, 400);
		group.setLayoutData( d7 );
	    ...
	    for (int i = 0; i < columnNameList.size(); i++)
	    {
		    ...
		    buttons[i].setText(columnNameList.get(i).toString());
